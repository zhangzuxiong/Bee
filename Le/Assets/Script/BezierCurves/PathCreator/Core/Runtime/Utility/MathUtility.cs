...
namespace PathCreation.Utility {
    public static class MathUtility {
        ...
        public static float MinAngle (Vector3 a, Vector3 b, Vector3 c)
        {
            ...
            // Debug.Log(JsonUtility.ToJson(p1)+"---"+JsonUtility.ToJson(p2));
            return Angle (p1,p2 );
        }
        public static float Angle(Vector3 from, Vector3 to)
        {
            float num = Mathf.Sqrt(from.sqrMagnitude * to.sqrMagnitude);
            // Debug.Log("------------"+num+"/"+from.sqrMagnitude+to.sqrMagnitude);
            // Debug.Log("------------"+num+"/"+JsonUtility.ToJson(from)+JsonUtility.ToJson(to));
            return (double) num < 1.00000000362749E-15 ? 0.0f : Mathf.Acos(Mathf.Clamp(Vector3.Dot(from, to) / num, -1f, 1f)) * 57.29578f;
        }
